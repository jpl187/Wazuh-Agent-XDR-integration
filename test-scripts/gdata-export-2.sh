...
# --- endpoints (adjust as needed) ---
AUTH_URL="https://your-api.example.com/auth/token"
EXPORT_URL="https://your-api.example.com/api/export"

# --- auth credentials (or source from a secrets file) ---
...
# extract token — adjust the jq filter to match your API's response shape
TOKEN="$(echo "$AUTH_RESPONSE" | grep -oP '(?<="token"\s*:\s*")[^"]+' )" \
  || die "failed to parse token from auth response: ${AUTH_RESPONSE:0:200}"

[[ -z "$TOKEN" ]] && die "parsed token is empty — check AUTH_URL or response format. response: ${AUTH_RESPONSE:0:200}"

# overwrite token file (atomic via tmp)
